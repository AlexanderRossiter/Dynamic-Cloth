...
Cloth
ClothLoader::load_cloth_from_file(std::string filepath)
{
...
void
ClothLoader::load_cloth_coordinates(Cloth& c, std::string& filepath)
{
  ...
  if (file.fail()) {
    ...
    throw std::exception(); // Nice, it just crashes if it can't read the file
                            // (－‸ლ)
  }
  ...
  // Store the indicies and randomise them, allows us to iterate through array
  // in random order.
...
void
ClothLoader::load_cloth_connectivity(Cloth& c, std::string& filepath)
{
  // Start by loading the points of the cloth.
