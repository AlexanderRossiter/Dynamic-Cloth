...
void
Cloth::print_cloth_connectivity()
{
  for (const Particle& c : points) {
...
void
Cloth::set_stiffness(float stiffness)
{
...
void
Cloth::set_damping(float damping)
{
  Cloth::damping = damping;
}

void
Cloth::draw(sf::RenderWindow& window)
{
...
Cloth::remove_particle(int idx)
{
  ...
  for (size_t i = 0; i < p.neighbours.size(); i++) {
    ...
    std::vector<int>::iterator it =
      std::find(n.neighbours.begin(), n.neighbours.end(), idx);
    if (it != n.neighbours.end()) {
      n.neighbours.erase(it);
      size_t rmv_idx = it - n.neighbours.begin();
      n.undeformed_distance_to_neighbours.erase(
        n.undeformed_distance_to_neighbours.begin() + rmv_idx);
    }
    /* n.neighbours.erase(std::remove(n.neighbours.begin(), n.neighbours.end(),
     * idx), n.neighbours.end());*/
    std::cout << "Neighbour " << i << ": "
              << util::vector_to_string(n.neighbours) << std::endl;
  }
  p.neighbours.clear();
}
