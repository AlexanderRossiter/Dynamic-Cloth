...
ClothSimulator::velocity_verlet_step(float delta_t)
{
  for (int p_idx : cloth.point_idicies) {
    ...
    if (!point.is_fixed) {
      point.x =
        point.x + point.xvel * delta_t + 0.5f * point.xacc * delta_t * delta_t;
      point.y =
        point.y + point.yvel * delta_t + 0.5f * point.yacc * delta_t * delta_t;
      point.z =
        point.z + point.zvel * delta_t + 0.5f * point.zacc * delta_t * delta_t;
    }
...
ClothSimulator::write_solution_to_file(int nstep)
{
  ...
  for (Particle& point : cloth.points) {
    file << std::setprecision(6) << point.x << "," << point.y << "," << point.z
         << std::endl;
  }
